/* Coming from GLIBC, test data is also in the RFC */
  ...
  for (cnt = 0; cnt < (int) (sizeof (tests) / sizeof (tests[0])); ++cnt)
    {
      ...
      osmo_md5_process_bytes (tests[cnt].input, strlen (tests[cnt].input), &ctx);
      osmo_md5_finish_ctx (&ctx, sum);
      result |= memcmp (tests[cnt].result, sum, 16);

      osmo_md5_init_ctx (&ctx);
      for (i = 0; tests[cnt].input[i] != '\0'; ++i)
	osmo_md5_process_bytes (&tests[cnt].input[i], 1, &ctx);
      osmo_md5_finish_ctx (&ctx, sum);
      result |= memcmp (tests[cnt].result, sum, 16);
    }

  if (result)
    printf("FAILED MD5test.\n");
  return result;
}
